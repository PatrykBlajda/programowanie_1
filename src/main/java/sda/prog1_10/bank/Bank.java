...
public class Bank {
    ...
    public boolean deleteAccoutn (Customer customer, Account account) {
        if(checkCustomerOnList(customer)) {
            List<Account> accounts = customer.getAccounts();
            if (accounts.contains(account)) {
                return removeAccountIfBalnzeZero(account, accounts);
            }
            return accountNotFound(account);
        }
        return customerNotFound(customer);
    }
    private boolean removeAccountIfBalnzeZero(Account account, List<Account> accounts) {
    ...
    public void printAccountList (Customer customer, boolean printBalance) {
        if (checkCustomerOnList(customer)) {
            List<Account> accounts = customer.getAccounts();
            accounts.stream() .forEach(a -> System.out.println("\t" + a.getAccountNumber() + " " + a.getAccountKind()
